...
Multiple independent Stations can run concurrently using `$set_daemons()` and `$add_parallel_group()`. This is useful for:
- Running multiple agents in parallel for comparison or consensus
- Executing independent analyses simultaneously
- Reducing total workflow latency by 2-4x for embarrassingly parallel tasks
...
1. **`$set_daemons(n)`** — Creates `n` parallel worker processes (typically `parallel::detectCores()`)
...
3. **Execution** — All parallel stations run concurrently in separate daemons, providing ~2-4x speedup

4. **Visualization** — `$visualize()` shows parallel routes in **red with ∥ symbol** to distinguish from sequential routes

### Performance

With 3 parallel agents analyzing the same topic:
- **Sequential**: ~12-15 seconds (3 API calls × 4-5s each)
- **Parallel**: ~4-5 seconds (all 3 calls simultaneously)
- **Speedup**: ~3x faster

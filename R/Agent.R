...
Agent <- R6::R6Class(
  ...
  public = list(
    ...
    #' @return Called for the side effect of generating and registering it an ellmer tool
    #' @examples
    #' \dontrun{
    #' openai_4_1_mini <- ellmer::chat(
    #'   name = "openai/gpt-4.1-mini",
    ...
    #' @description
    #' Export all registered tools to a JSON file for later reuse.
    #' Serializes tool functions, metadata, and parameter specifications.
    #' @param file_path Character string specifying the file path for the JSON export.
    #' Defaults to `"<agent_name>_tools.json"` in the current directory.
    #' @examples
    #' \dontrun{
    #' openai_4_1_mini <- ellmer::chat(
    #'   name = "openai/gpt-4.1-mini",
    ...
    #' )
    #' agent$register_tools(list(change_directory_tool(), list_files_tool()))
    #' agent$export_tools("my_tools.json")
    #' }
    export_tools = function(
      file_path = paste0(getwd(), "/", paste0(self$name, "_tools.json"))
    ) {

      checkmate::assert_string(file_path)

      if (length(self$tools) == 0) {
        cli::cli_alert_warning("No tools registered to export.")
        return(invisible(NULL))
      }

      cli::cli_alert_info("Exporting {length(self$tools)} tool{?s}...")

      tools_export <- lapply(names(self$tools), function(tool_name) {
        ...
        # Extract the function - ellmer::ToolDef stores it in .Data slot
        tool_function <- tryCatch({
          # Try accessing via .Data (S7 standard for wrapper objects)
          if (!is.null(tool@.Data)) {
            tool@.Data
          } else {
          ...
          cli::cli_alert_danger("Failed to extract function from tool '{tool_name}': {e$message}")
          NULL
        })

        if (is.null(tool_function)) {
          cli::cli_abort("Could not extract function from tool '{tool_name}'")
        }

        # Serialize the function as text
        tool_data$function_code <- paste(deparse(tool_function), collapse = "\n")

        # Extract argument specifications
        if (length(tool@arguments) > 0) {
          tool_data$arguments <- lapply(names(tool@arguments), function(arg_name) {
            arg_spec <- tool@arguments[[arg_name]]

            arg_data <- list(
              name = arg_name,
              type = class(arg_spec)[1],
              description = if (!is.null(arg_spec@description)) arg_spec@description else "",
              required = if (!is.null(arg_spec@required)) arg_spec@required else FALSE
            )

            # Handle type-specific properties
            if (inherits(arg_spec, "ellmer::TypeEnum")) {
              arg_data$values <- arg_spec@values
            } else if (inherits(arg_spec, "ellmer::TypeArray")) {
              arg_data$items_type <- class(arg_spec@items)[1]
            }

            arg_data
          })
          names(tool_data$arguments) <- names(tool@arguments)
        } else {
          tool_data$arguments <- list()
        }

        tool_data
      })

      names(tools_export) <- names(self$tools)
      ...
      cli::cli_alert_success("Tools exported to {file_path}")
      invisible(file_path)
    },

    #' @description
    #' Import and register tools from a JSON file previously exported with export_tools().
    #' Reconstructs tool functions and ellmer tool definitions from serialized data.
    #' @param file_path Character string specifying the path to the JSON file containing tools.
    #' Defaults to `"<agent_name>_tools.json"` in the current directory.
    #' @param overwrite Logical; if TRUE, existing tools with the same name will be replaced.
    #' If FALSE, existing tools are preserved. Default: FALSE.
    #' @examples
    #' \dontrun{
    #' openai_4_1_mini <- ellmer::chat(
    #'   name = "openai/gpt-4.1-mini",
    ...
    #' agent$load_tools("my_tools.json")
    #' agent$list_tools()
    #' }
    load_tools = function(
      file_path = paste0(getwd(), "/", paste0(self$name, "_tools.json")),
      ...
      cli::cli_alert_info("Importing tools from {file_path}...")

      import_data <- jsonlite::read_json(file_path, simplifyVector = FALSE)

      if (is.null(import_data$tools) || length(import_data$tools) == 0) {
        cli::cli_alert_warning("No tools found in file.")
        return(invisible(NULL))
      }
      ...
      for (tool_name in names(import_data$tools)) {
        tool_data <- import_data$tools[[tool_name]]

        # Skip if tool already exists and overwrite is FALSE
        if (!overwrite && tool_name %in% names(self$tools)) {
          cli::cli_alert_warning("Tool '{tool_name}' already exists, skipping (use overwrite=TRUE to replace)")
          skipped_count <- skipped_count + 1
          next
        }

        tryCatch({
          # Reconstruct the function from code
          tool_function <- eval(parse(text = tool_data$function_code))

          # Reconstruct argument specifications
          arguments_list <- list()

          if (length(tool_data$arguments) > 0) {
            for (arg_name in names(tool_data$arguments)) {
              arg_spec <- tool_data$arguments[[arg_name]]

              # Create the appropriate type based on serialized type
              type_obj <- switch(
                arg_spec$type,
                "ellmer::TypeString" = ellmer::type_string(
                  description = arg_spec$description,
                  required = arg_spec$required
                ),
                "ellmer::TypeNumber" = ellmer::type_number(
                  description = arg_spec$description,
                  required = arg_spec$required
                ),
                "ellmer::TypeInteger" = ellmer::type_integer(
                  description = arg_spec$description,
                  required = arg_spec$required
                ),
                "ellmer::TypeBoolean" = ellmer::type_boolean(
                  description = arg_spec$description,
                  required = arg_spec$required
                ),
                "ellmer::TypeEnum" = ellmer::type_enum(
                  values = arg_spec$values,
                  description = arg_spec$description,
                  required = arg_spec$required
                ),
                "ellmer::TypeArray" = {
                  # For arrays, we need to create the items type first
                  items_type <- switch(
                    arg_spec$items_type,
                    "ellmer::TypeString" = ellmer::type_string(),
                    "ellmer::TypeNumber" = ellmer::type_number(),
                ...
                "ellmer::TypeObject" = ellmer::type_object(
                  description = arg_spec$description,
                  required = arg_spec$required
                ),
              ...
              arguments_list[[arg_name]] <- type_obj
            }
          }

          # Create the tool using ellmer::tool()
          reconstructed_tool <- ellmer::tool(
            tool_function,
            name = tool_data$name,
            description = tool_data$description,
            arguments = if (length(arguments_list) > 0) arguments_list else NULL
          )

          tools_to_register[[tool_name]] <- reconstructed_tool

          cli::cli_alert_success("Reconstructed tool: {tool_name}")

        }, error = function(e) {
          cli::cli_alert_danger("Failed to reconstruct tool '{tool_name}': {e$message}")
        })
      }

      # Register all successfully reconstructed tools
      if (length(tools_to_register) > 0) {
        for (tool_name in names(tools_to_register)) {
          self$tools[[tool_name]] <- tools_to_register[[tool_name]]
        }

        private$.update_llm_tools()

        cli::cli_alert_success(
          "Successfully imported {length(tools_to_register)} tool{?s}"
        )

        if (skipped_count > 0) {
          cli::cli_alert_info("Skipped {skipped_count} existing tool{?s}")
        }
      } else {
        cli::cli_alert_warning("No tools were successfully imported")
      }

      invisible(tools_to_register)
    },
